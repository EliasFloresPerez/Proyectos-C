...
int af,ac,bf,bc,game,rot,afin,bfin; //variables del ataque (lo se muchas variables y pude reutilizar ;v)
srand(time(0));
...
for(int x=0;x<4;x++)
{
	for(int y=0;y<4;y++)
	{
		...
		if(arregloA[x][y]==0||arregloA[x][y]==1)
		{
		cout<<"["<<arregloA[x][y]<<"]";
	    }
	}
